...
def Save_to_S3(bucket_name, prefix, table_name, file_format):
    ...
    postgres_hook = PostgresHook(postgres_conn_id='ec2-postgres')
    # SQL query
...
def Save_to_S3_1(bucket_name, prefix, table_name, file_format):
    # Postgres hook to execute SQL
    postgres_hook = PostgresHook(postgres_conn_id='ec2-postgres')
    # SQL query
    sql_query = "SELECT emp_id FROM {}".format(table_name)
    # Fetch data using the SQL query
    df = postgres_hook.get_pandas_df(sql_query)
